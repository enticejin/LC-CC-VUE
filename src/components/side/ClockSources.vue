<template>
 <div>
   ...
   <el-table
       size="small"
       :data="clockSourceList.slice((currentPage-1)*pageSize,currentPage*pageSize)"
       ...
       align="center"
         ...
         <template slot-scope="scope">
           <el-popover trigger="hover" placement="right">
             ...
             <p><strong>是否在线</strong>：{{ scope.row.enable }}</p>
     ...
     <!-- 编辑 -->
     <el-form :model="clockSourceForm" :anchor="clockSourceList" ref="clockSourceForm" label-width="70px" class="demo-clockSourceForm" size="medium">
       <el-dialog
         title="时钟源编辑"
         :append-to-body='true'
     ...
     <div class="block" style="margin-top:15px;">
      <el-pagination align='center' @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[1,5,10,20]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="clockSourceList.length">
      </el-pagination>
     </div>
 </div>
</template>
...
<script>
  export default{
    ...
    methods: {
      ...
      //获取时钟
      getclockSources: function(){
        var me = this;
        me.clockSourceList = []
        me.axios.get('/clocksources', {
          headers: {'SessionId': $cookies.get('sessionid'),'Authorization': $cookies.get('accessToken')}
        })
        .then(response => {
          me.clockSourceList = response.data;
      ...
      refreshData () {
        ...
        .then(response => {
          this.clockSourceList = []
          for(var i =0 ;i < response.data.length;i++){
            this.clockSourceList.push(response.data[i])
          }
        })
        .catch(error => {
          console.log(error);
      ...
      onSearch(){
       ...
       updateAnchor(){
         ...
         .then(response => {
           this.clockSourceList = response.data
           console.log(this.clockSourceList)
           this.$message({
           type: 'success',
           message: '更新成功'
           }),
